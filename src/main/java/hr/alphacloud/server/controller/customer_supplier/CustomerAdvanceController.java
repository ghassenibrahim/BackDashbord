...
import hr.alphacloud.server.model.command.customer_supplier.CustomerAdvanceSaveCommand;
...
@RestController
@RequestMapping("api/customer-advances")
public class CustomerAdvanceController {

    private final CustomerSupplierService<CustomerAdvance> customerAdvanceService;

    public CustomerAdvanceController(CustomerSupplierService<CustomerAdvance> customerAdvanceService) {
    ...
    public ApiBaseDTO<List<CustomerSupplierDTO>> filter(@RequestBody @Valid ApiBasePageCommand<CustomerSupplierFilterCommand> command) {
        return this.customerAdvanceService.filter(command, CustomerAdvance.class);
    }

    @PostMapping("/save")
    public ApiBaseDTO<Boolean> save(@RequestBody @Valid ApiBaseCommand<CustomerAdvanceSaveCommand> command) {
        return this.customerAdvanceService.save(command);
    }

    @PostMapping("/delete")
    public ApiBaseDTO<Boolean> delete(@RequestBody @Valid ApiBaseCommand<DeleteCommand> command) {
        return this.customerAdvanceService.delete(command);
    }
    ...
    public ApiBaseDTO<List<ReportTableDataDTO>> getDataTable(@RequestBody @Valid ApiBasePageCommand<CustomerSupplierFilterCommand> command) {
        return this.customerAdvanceService.getDataTable(command, CustomerAdvance.class);
    }
}
